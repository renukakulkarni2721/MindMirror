...
export default function Daily() {
    ...
    useEffect(() => {
        const fetchReflection = async () => {
            ...
            try {
                // Fetch via API to bypass client security rules
                console.log(`Fetching daily reflection for ${date}`);
                const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}/reflection/${user.uid}/${date}`);
